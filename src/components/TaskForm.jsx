...
const TaskForm = ({ data, setData, filter, setFilter }) => {
  ...
  const validate = () => {
    ...
    if (!form.description.trim()) {
      ...
      newErrors.description =`Please Enter ${200 - form.description.trim().length} more character`
    }
  ...
  const addData = (e) => {
    ...
    // code for update
    if(id){
      var updatedTask = data.map((item)=>
        item.id === Number(id) ? {...form, id:Number(id)}
        : item
      )
      setData(updatedTask);
    }
  ...
  useEffect (()=>{
    if(id){
      const selectedTask = data.find((item) => item.id === Number(id))
